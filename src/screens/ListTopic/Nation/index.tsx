...
const ListTopicNation = (props: any) => {
  const {nation} = props.route.params;
  ...
  const firstLoad = async () => {
    ...
    try {
      const response = await AppAPI.getTopicByNation(nation, 1);
  ...
  const getList = async () => {
    GlobalService.showLoading();
    try {
      const response = await AppAPI.getTopicByNation(nation, page + 1);
      const data = response?.data;
      GlobalService.hideLoading();
      if (data?.success) {
